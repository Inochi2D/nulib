...
*/
module nulib.conv.algorithms.eisel_lemire;
...
/**
    Converts a mantissa, base-10 exponent and negative flag into
    a floating point number.

    Params:
        mantissa =  The mantissa of the floating point number.
        exp10 =     The exponent of the floating point number.
        negative =  Whether the number is negative.
        result =    The variable to store the parsed result into.

    Returns:
        The floating point representation of the input values,
        $(D ok) is set depending on whether conversion succeeded.
*/
bool eiselLemire(FT)(ulong mantissa, long exp10, bool negative, ref FT result) @nogc nothrow pure 
if (__traits(isFloating, FT)) {
    ...
    // Calculation done.
    ulong retBits = retExp2<<(FT.mant_dig-1) | (retMan&~(ulong.max<<FT.mant_dig-1));
    result = *(cast(FT*)&retBits);
    if (negative)
        result = -result;
    return true;
}
...
u128_t mulu64(ulong x, ulong y) @nogc nothrow pure {
    u128_t result;
