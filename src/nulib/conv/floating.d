/**
    ...
    and Experience 51 (8), 2021 (https://arxiv.org/abs/2101.11408)
    as described by Nigel Tao (https://nigeltao.github.io/blog/2020/eisel-lemire.html)
...
*/
T parseFloat(T, S)(S source, char sep='.') @nogc nothrow
if (__traits(isFloating, T)) {
    if (source.length == 0)
        return T.nan;

    // TODO: Select between algorithms.
    return T.nan;
}
...
unittest {
    ...
    foreach(i; 0..100_000) {
        float ft = uniform01();
        assert(parseFloat!float(ft.text) == ft);
    }
}
...
unittest {
    ...
    foreach(i; 0..100_000) {
        double ft = uniform01();
        assert(parseFloat!double(ft.text) == ft);
    }
}
...
// Implements a generic eisel Lemire.
FT eiselLemire(FT)(ulong mantissa, long exp10, bool negative) {
    if (mantissa == 0) {
        if (negative)
            return -0;
        return 0;  
    ...
    // Out of range.
    if (exp10 < EL_POW10_MIN_EXP || EL_POW10_MAX_EXP < exp10)
        return FT.init;

    enum ftExpBias = FT.max_exp-1;
    enum ftExpBiasHalf = ftExpBias/2;
    enum ftShift = 64-(FT.sizeof*8);
    enum ftMask = (ftExpBiasHalf<<ftShift) + (cast(ulong)1<<ftShift)-1;
    enum ftShift54 = 63-(FT.mant_dig+1);
    enum ftSubnormMask = (FT.max_exp*2)-1;

    // Normalize.
    long clz = nu_bsr(mantiassa);
    mantissa <<= clz;
    ...
    if ((x.hi & ftMask) == ftMask && x.lo + mantissa < mantissa) {
        ...
        if ((merged.hi & ftMask) == ftMask && merged.lo+1 == 0 && y.lo + mantissa < mantissa)
            return FT.init;
    ...
    // Shift to 54 bits.
    ulong msb = x.hi >> 63;
    ulong retMan = x.hi >> (msb + ftShift54);

    // Halfway Ambiguity.
    if (x.lo == 0 && (x.hi & ftExpBiasHalf) == 0 && (retMan & 3) == 1) {
        return FT.init;
    }
    ...
    // Handle sub-normal f64 space.
    if (retExp2-1 >= ftSubnormMask-1) {
        return FT.init;
    }

    size_t retBits = retExp2<<(FT.mant_dig-1) | (retMan&~(ulong.max<<FT.mant_dig-1));
    retBits |= (cast(ulong)neg << (FT.sizeof*8)-1);
    return *(cast(FT*)&retBits);
}
...
// Multiplies 2 64 bits numbers and stores the result
// in a 128 bit storage type.
u128_t mulu64(ulong x, ulong y) @nogc pure {
    u128_t result;
...
// The following LUT is generated by 
// https://github.com/google/wuffs/blob/788479dd64f35cb6b4e998a851acb06ee962435b/script/print-mpb-powers-of-10.go
// TODO: Generate this with ctfe?

// Helper that converts exp10 to exp2
pragma(inline, true)
long toExp2(long exp10) @nogc nothrow pure => (217_706 * exp10) >> 16;

// Ranges for LUT
enum EL_POW10_MIN_EXP = -348;
enum EL_POW10_MAX_EXP = 347;

// LUT of powers of 10 for optimisation.
__gshared const ulong[2][696] EL_POW10_TABLE = [
